...
//                || (remarks.indexOf('（') > -1 && remarks.indexOf('）') > -1)) {
                ) {if ((remarks.indexOf('(') == remarks.lastIndexOf('(')
                    && remarks.indexOf(')') == remarks.lastIndexOf(')'))
                    || (remarks.indexOf('（') == remarks.lastIndexOf('（')
                    && remarks.indexOf('）') == remarks.lastIndexOf('）'))) {
                // 中英文括号成对出现时生效
                String enums = null;
                if (remarks.indexOf('(') == -1) {
                    enums = remarks.substring(remarks.indexOf('（') + 1, remarks.indexOf('）'));
                } else {
                    enums = remarks.substring(remarks.indexOf('(') + 1, remarks.indexOf(')'));
                }
                String split = ";";
    ...
    public String getCommentWithoutEnums(){
        if(this.remarks.indexOf('(') != -1 || this.remarks.indexOf('（') != -1){
            return this.remarks.substring(0, Math.max(this.remarks.indexOf('('), this.remarks.indexOf('（')));
        }
        return this.remarks;
    }
}
